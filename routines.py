...
class SpeedFlip():
    ...
    def run(self, agent, dt: float):
        ...
        elif self.timer < self.FIRST_JUMP_DURATION + self.BETWEEN_JUMPS_DELAY:
            self.controls.jump = False
            self.controls.pitch = 1.0

        elif (
            self.timer
            < self.FIRST_JUMP_DURATION + self.BETWEEN_JUMPS_DELAY + self.SECOND_JUMP_DURATION
        ):
            self.controls.jump = True
            self.controls.pitch = -1.0
        ...
        self.finished = (self.timer > self.TIMEOUT) or (
...
class demo():
    #Tries to demo a designated target
    def __init__(self, target, direction = 1):
        self.target = target
    ...
    def run(self,agent):
        ...
        if agent.me.boost < 20:
            ...
            agent.pop()
        elif abs(angles[1]) < 0.05 and velocity > 600 and velocity < 900 and distance_remaining / velocity > 2.0:
