...
class GoslingAgent(BaseAgent):
    ...
    def text(self, text, color=None):
        color = color if color != None else [255, 255, 255]
        self.renderer.draw_string_3d(self.me.location, 1, 1, text, self.renderer.create_color(255, *color))

    def debug_info(self):
        # Draws car information on the screen
        color = self.renderer.white()
        self.renderer.draw_string_2d(50, 40, 1, 1, "Useful Information", color)
        self.renderer.draw_string_2d(50, 55, 1, 1, "-----------------------", color)
        self.renderer.draw_string_2d(50, 70, 1, 1, f"Boost: {self.me.boost}", color)
        self.renderer.draw_string_2d(50, 90, 1, 1, f"Airborne: {self.me.airborne}", color)
        self.renderer.draw_string_2d(50, 110, 1, 1, f"Me Ball Distance: {(self.me.location - self.ball.location).magnitude()}", color)
        self.renderer.draw_string_2d(50, 130, 1, 1, f"Foe Ball Distance: {(self.foes[0].location - self.ball.location).magnitude()}", color)
        self.renderer.draw_string_2d(50, 150, 1, 1, f"Me Foe Distance: {(self.me.location - self.foes[0].location).magnitude()}", color)
        self.renderer.draw_string_2d(50, 170, 1, 1, f"Me Foe Goal Distance: {(self.me.location - self.foe_goal.location).magnitude()}", color)
        self.renderer.draw_string_2d(50, 190, 1, 1, f"Me Friend Goal Distance: {(self.me.location - self.friend_goal.location).magnitude()}", color)
        self.renderer.draw_string_2d(50, 210, 1, 1, f"Foe Foe Goal Distance: {(self.foes[0].location - self.friend_goal.location).magnitude()}", color)
        self.renderer.draw_string_2d(50, 230, 1, 1, f"Foe Friend Goal Distance: {(self.foes[0].location - self.foe_goal.location).magnitude()}", color)
    
    def debug_stack(self):
        # Draws the stack on the screen
        color = self.renderer.white()
        self.renderer.draw_string_2d(50, 300, 1, 1, "Action Stack", color)
        self.renderer.draw_string_2d(50, 315, 1, 1, "-----------------------", color)
        for i in range(len(self.stack) - 1, -1, -1):
            text = f"{self.name}: {self.stack[i].__class__.__name__}"
            self.renderer.draw_string_2d(50, 315 + (20 * (len(self.stack) - i)), 1, 1, text, color)
